---
title: "Færre detaljer gir mer rom for omtanke"
description: "Hver funksjon og tilstand krever oppmerksomhet. Begrensninger kan gi rom for bedre håndverk."
publishDate: 2026-09-22
tags:
  - Rauting
  - interaksjonsdesign
  - produktutvikling
draft: false
---

Limit the number of details undersøker sammenhengen mellom kompleksitet og håndverk. «Detaljer» omfatter her funksjoner, tilstander og oppførsel, langt utover det rent visuelle.

<a class="raut-source-card" href="https://design.lightspark.com/limit-the-number-of-details" target="_blank" rel="noopener noreferrer">
  <span class="raut-source-card__meta">FAGLIG LESETIPS · design.lightspark.com</span>
  <strong class="raut-source-card__title">Limit the number of details</strong>
  <span class="raut-source-card__description">Hver funksjon og tilstand krever oppmerksomhet. Begrensninger kan gi rom for bedre håndverk.</span>
  <span class="raut-source-card__action">Les originalartikkelen <span aria-hidden="true">↗</span><span class="sr-only"> (åpnes i ny fane)</span></span>
</a>

## Key takeaways

- En ekstra knapp krever beslutninger om plassering, tekst, tilbakemelding og forholdet til andre handlinger.
- Flere funksjoner øker både vedlikeholdsarbeidet og det brukeren må forstå.
- Et begrenset antall tydelige kontroller kan gi rom for mer gjennomarbeidet betjening.
- Kompleksitet kan være nødvendig og verdifull. Den bør begrunnes i hva produktet skal hjelpe brukeren å gjøre, fremfor å vokse ved stadig nye små tillegg.

## Ding! 💡

Jeg vil ta med meg dette når det frister å legge enda en funksjon i hagen. Hvert tillegg kan virke uskyldig alene, men til slutt konkurrerer de om både min og leserens oppmerksomhet. Jeg vil heller være tydelig på hvilken oppgave noe løser, og gi de viktigste detaljene nok stell. Enkelhet må merkes i bruken, ikke bare i skjermbildet.

---

[Raut#196](https://raut.no/196), 22. september 2026
