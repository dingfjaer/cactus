---
title: "PACT: fire spørsmål før du begynner å tegne"
description: "Et enkelt rammeverk for å forstå mennesker, aktiviteter, kontekst og teknologi før du velger løsning."
publishDate: 2025-03-31
tags:
  - Rauting
  - brukerinnsikt
  - ux
draft: false
---

Pavel Samsonov beskriver PACT som en måte å skaffe oversikt i et ukjent fagområde. Rammeverket hjelper også teamet å oppdage at det ikke nødvendigvis deler samme problemforståelse.

<a class="raut-source-card" href="https://medium.com/design-bootcamp/if-you-dont-know-where-to-start-research-start-with-pact-1ae4b6802134" target="_blank" rel="noopener noreferrer">
  <span class="raut-source-card__meta">FAGLIG LESETIPS · medium.com</span>
  <strong class="raut-source-card__title">If you don’t know where to start research, start with PACT</strong>
  <span class="raut-source-card__description">Et enkelt rammeverk for å forstå mennesker, aktiviteter, kontekst og teknologi før du velger løsning.</span>
  <span class="raut-source-card__action">Les originalartikkelen <span aria-hidden="true">↗</span><span class="sr-only"> (åpnes i ny fane)</span></span>
</a>

## Key takeaways

- PACT står for People, Activities, Context og Technology: hvem som bruker løsningen, hva de gjør, under hvilke forhold og med hvilken teknologi.
- Start gjerne med fagpersoner som kjenner området, og samle svarene i et felles dokument.
- Spør hvordan dere vet det dere hevder. Gjentatte antakelser er ikke det samme som flere uavhengige observasjoner.
- Uenighet og kunnskapshull gir retning for videre brukerinnsikt. Oppfølgingsspørsmål gjør samtaler mer nyttige enn et skjema med korte svar.

## Hva Chatty tenker at Ding tenker

Jeg liker at PACT gjør det legitimt å stille de tilsynelatende enkle spørsmålene. De er ofte akkurat det som forsvinner når alle vil raskt til en løsning. Jeg ville brukt rammeverket til å samle både det vi vet og det vi bare antar, før første skisse får for stor innflytelse på resten av arbeidet.

---

[Raut#139](https://raut.no/139), 31. mars 2025
