---
title: "Din konto, min konto – eller bare konto?"
description: "Små pronomen i grensesnittet påvirker hvem som snakker, og hvor lett teksten er å forstå."
publishDate: 2025-09-22
tags:
  - Rauting
  - innholdsdesign
  - ux
draft: false
---

Adam Silver tar opp et spørsmål som stadig kommer tilbake i grensesnitt: Skal vi skrive «min» eller «din»? Ofte er den beste starten å undersøke om pronomenet trengs.

<a class="raut-source-card" href="https://adamsilver.io/blog/your-vs-my-in-user-interfaces/" target="_blank" rel="noopener noreferrer">
  <span class="raut-source-card__meta">FAGLIG LESETIPS · adamsilver.io</span>
  <strong class="raut-source-card__title">“Your” vs “My” in user interfaces</strong>
  <span class="raut-source-card__description">Små pronomen i grensesnittet påvirker hvem som snakker, og hvor lett teksten er å forstå.</span>
  <span class="raut-source-card__action">Les originalartikkelen <span aria-hidden="true">↗</span><span class="sr-only"> (åpnes i ny fane)</span></span>
</a>

## Key takeaways

- Når eierskapet er opplagt, kan «Konto», «Bestillinger» eller «Saker» være tilstrekkelig.
- Hvis egne og andres saker må skilles, blir ordvalget mer betydningsfullt.
- «Dine saker» fungerer naturlig når tjenesten snakker til brukeren, også i veiledninger, varsler og kundestøtte.
- «Mine» kan passe når valget uttrykker brukerens egen beskjed til systemet. Hvem som snakker, bør styre perspektivet.

## Hva Chatty tenker at Ding tenker

Jeg liker at en liten tekstbeslutning blir vurdert utenfor selve skjermbildet. Et navn må også fungere når noen forklarer det over telefon eller skriver om det i en e-post. Neste gang dette spørsmålet dukker opp, vil jeg prøve teksten i en hel setning. Da blir det ofte lettere å høre om perspektivet henger sammen.

---

[Raut#158](https://raut.no/158), 22. september 2025
